...
(function(window, doc) {
    ...
    var createMenuItem = function(id, name, data,color, callback) {
        // 创建菜单项
        ...
        color && span.setAttribute("style", "background-color:"+color);
        if(data && data.length > 0){
            for(var i in data){
                var item = data[i];
                span.setAttribute("data-"+item['name'], item['val']);
            }
        }
        span.innerText = name;
    ...
    var createMenu = function(index, menuArr) {
        ...
        for (var i = 0; i < menuArr.length; i++) {
            ...
            // 创建菜单项
            var oneMenu = createMenuItem(menuItem.id, menuItem.name, menuItem.data,menuItem.color, menuItem.callback);
            if (!parent) {
     ...
     */
    window.contextMenu = function(objS, menuArr) {
        ...
        Object.keys(objS).forEach(function(i){
            ...
            if(menuArr.length < 1){
                ...
                obj.oncontextmenu = function(event) {
                    ...
                    var objS = document.querySelectorAll(".web-context-menu");
                    if(objS.length > 0){
                        Object.keys(objS).forEach(function(elem, index) {
                            objBody.removeChild(objS[elem]);
                        });
                    }
                    //创建新的右键目录
                    ...
                    if (!menu || menu.length < 1) {
                        ...
                        menu.oncontextmenu=function(event){return false;}//禁止右键目录上还可以右键
                    }
                    showMenu(event, menu);
